...
import type { ServerStatusJson, Json } from "./types.js";
...
type ServiceState = "running" | "stopped" | "restarting" | "unknown";
...
function getDockerServiceStatus(): Record<string, ServiceState> {
...
function generateFakeServiceStatus(): Record<string, ServiceState> {
...
function getSystemMetrics(): { cpu_percent: number; mem_percent: number } {
  try {
...
function getMoavVersion(): string | null {
  try {
...
function generateRealStatus(): ServerStatusJson {
...
export function startHeartbeat(serverId: string) {
  async function beat() {
    ...
    const updateData: Record<string, unknown> = {
      ...
      status_json: status as unknown as Json,
    };
