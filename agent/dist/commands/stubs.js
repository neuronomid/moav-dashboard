...
`,
    ...
    if (payload.server_id && payload.username) {
        await supabase
            .from("vpn_users")
            .update({ config_raw: mockConfig, status: 'active' }) // Cast to any because generated types might not be up to date in agent
...
export const userUpdate = async (payload) => {
    ...
    if (payload.vpn_user_id) {
        ...
        if (payload.data_limit_gb !== undefined)
            updates.data_limit_gb = payload.data_limit_gb;
        if (payload.expires_at !== undefined)
            updates.expires_at = payload.expires_at;
        if (payload.access_policy !== undefined)
            updates.access_policy = payload.access_policy;
        await supabase
            .from("vpn_users")
            .update(updates)
