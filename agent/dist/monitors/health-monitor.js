 ...
 */
function getServiceLogs(serviceName, lines = 50) {
    try {
        // Service name comes from Docker's own output, not user input
        const output = execSync(`docker compose logs --tail ${lines} ${serviceName}`, {
            cwd: "/opt/moav",
            encoding: "utf-8",
            timeout: 10000,
            stdio: ["pipe", "pipe", "pipe"],
 ...
 */
async function logRemediationEvent(serviceName, issue, action, success, details) {
    if (!serverId)
        return;
    try {
 ...
 */
async function checkAndRemediateService(serviceName, serviceInfo) {
    ...
    await logRemediationEvent(serviceName, issue, action, success, success ? undefined : `Recent logs:\n${logs}`);
    if (success) {
        console.log(`[health-monitor] ✓ Successfully remediated ${serviceName} (attempt ${history.remediationAttempts})`);
        // Reset history on success
        serviceHistory.delete(serviceName);
    }
    else {
        console.error(`[health-monitor] ✗ Failed to remediate ${serviceName} (attempt ${history.remediationAttempts}/${MAX_REMEDIATION_ATTEMPTS})`);
    }
}
/**
 ...
 */
export function startHealthMonitor(serverIdParam) {
